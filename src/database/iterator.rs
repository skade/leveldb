...
///
/// Returns key and value as a tuple.
pub struct RevIterator<'a, K: Key<'a> + 'a> {
    start: bool,
    // Iterator accesses the Database through a leveldb_iter_t pointer
    // but needs to hold the reference for lifetime tracking
    #[allow(dead_code)]
...
///
/// Returns just the keys.
pub struct RevKeyIterator<'a, K: Key<'a> + 'a> {
...
pub struct ValueIterator<'a, K: Key<'a> + 'a> {
    inner: Iterator<'a, K>,
}
...
impl<'a, K: Key<'a>> Iterable<'a, K> for Database<'a, K> {
    fn iter(&'a self, options: ReadOptions<'a, K>) -> Iterator<K> {
...
#[allow(missing_docs)]
pub trait LevelDBIterator<'a, K: Key<'a>> {
    ...
    fn key(&self) -> K {
        unsafe {
            ...
            K::from(from_raw_parts(value, length as usize))
        }
    }
...
impl<'a, K: Key<'a>> Iterator<'a, K> {
    fn new(database: &'a Database<'a, K>, options: ReadOptions<'a, K>) -> Iterator<'a, K> {
        unsafe {
...
impl<'a, K: Key<'a>> LevelDBIterator<'a, K> for Iterator<'a, K> {
...
impl<'a, K: Key<'a>> LevelDBIterator<'a, K> for RevIterator<'a, K> {
...
impl<'a, K: Key<'a>> KeyIterator<'a, K> {
    fn new(database: &'a Database<'a, K>, options: ReadOptions<'a, K>) -> KeyIterator<'a, K> {
        KeyIterator {
...
impl<'a, K: Key<'a>> ValueIterator<'a, K> {
    fn new(database: &'a Database<'a, K>, options: ReadOptions<'a, K>) -> ValueIterator<'a, K> {
        ValueIterator {
            inner: Iterator::new(database, options),
        }
    }
...
macro_rules! impl_leveldb_iterator {
    ($T:ty, $RevT:ty) => {
        impl<'a, K: Key<'a>> LevelDBIterator<'a, K> for $T {
            type RevIter = $RevT;
...
macro_rules! impl_iterator {
    ($T:ty, $Item:ty, $ItemMethod:ident) => {
        impl<'a, K: Key<'a>> iter::Iterator for $T {
            type Item = $Item;
